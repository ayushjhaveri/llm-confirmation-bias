...
def strip_thinking(text: Optional[str]) -> Optional[str]:
    ...
    # Case 1: Standard explicit block
    if re.search(r"(?i)<think>", t) and re.search(r"(?i)</think>", t):
        t = THINK_BLOCK_RE.sub("", t)

    # Case 2: "closing-only" style: reasoning then </think> then answer/action
    elif re.search(r"(?i)</think>", t):
        # remove everything up to and including the first </think>
        t = re.sub(r"(?is)^.*?</think>\s*", "", t, count=1)

    # Cleanup stray tags/markers
...
class PromptsAgent(Agent):
    ...
    def act(self, obs, game_state):
        ...
        try:
            ...
            response_msg_stripped = strip_thinking(response_msg)
            action = extract_action(response_msg_stripped)
        ...
        act_info = {
            ...
            "response_message_stripped": response_msg_stripped,
    ...
    def answer_tf(self, question: str, env: Optional[object] = None):
        ...
        try:
            ...
            response_msg_stripped = strip_thinking(response_msg)
            response_usage = response.usage
            answer_str = extract_action(response_msg_stripped)
