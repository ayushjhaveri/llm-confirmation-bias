...
class BlicketFunctionSet:
    ...
    @staticmethod
    def xor(item_states: np.ndarray, blicket_indices: np.ndarray):
        """
        Exclusive OR over blickets (generalized): True iff EXACTLY ONE blicket is on the machine.
        - For 2 blickets, this matches standard XOR.
        - For >2, this is the "exactly-one" generalization.
        """
        blicket_states = item_states[blicket_indices]
        return np.sum(blicket_states) == 1
...
ENV_DESC = "You are in a room. You see a device at the center of this room. "\
"\n\nThere are also #NUM_OBJECT# objects scattered around the room. #OBJECT_DESC# "\
"\n\n#MACHINE_STATE# "\
"You wonder if there is a relationship between the objects and the device."
...
class BlicketTextEnv:
    ...
    def _get_object_description(self):
        ...
        for i in range(self.num_objects):
            loc = "on the device" if self._state[i] else "on the floor"
    ...
    def _get_machine_description(self):
        light_state = "on" if self._state[-1] else "off"
        return "The device is " + light_state + "."
