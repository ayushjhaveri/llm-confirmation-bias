...
class ThinkOppositesPriorExperiment:
    NAME = "think-in-opposites_prior"

    def __init__(
        self,
        hidden_rule_name: str,
        hidden_rule_pred,
        seed_example: Tuple[int, int, int],
        candidate_rule_names: List[str],
    ):
        self.rule_name = hidden_rule_name
        self.rule_pred = hidden_rule_pred
        self.seed = seed_example
        self.candidate_rule_names = candidate_rule_names
        self.max_pairs = 45
        self.supports_think = False
        self.debug_retries = False

    def _candidate_rules_text(self) -> str:
        lines = []
        for i, name in enumerate(self.candidate_rule_names, start=1):
            tag = chr(ord("a") + i - 1)
    ...
    def _initial_prompt(self) -> str:
        a, b, c = self.seed
        rule_set_text = self._candidate_rules_text()
        ...
        return (
            ...
            "Before proposing each new triple, look at the example and think about one feature it has.\n"
            "Then imagine a triple that is opposite with respect to that feature, and test whether it still conforms with the rule.\n"
            "If it does, that feature is probably irrelevant; if it does not, that feature might be crucial.\n\n"
            "In short: to discover the rule, test triples that both confirm and contradict your current idea.\n\n"
            "At each turn, you may perform one of two actions:\n\n"
            "Test: propose a new set of three integers, denoted [a,b,c].\n"
            "You will then be told whether that triple conforms with the hidden rule (\"YES\" or \"NO\").\n\n"
            "Announce: state what you believe the hidden rule is.\n"
            ...
            "  Check: [a,b,c]\n\n"
            "The hidden rule is guaranteed to be one of the candidate rules listed below. Before seeing any test results, you should assume that each candidate rule is equally likely.\n\n"
            f"{rule_set_text}\n\n"
    ...
    def _retry_hint(self, expected: str) -> str:
        if expected == "announce":
            return (
                "Turn - Announce\n"
                "Remember: Output EXACTLY ONE LINE:\n"
                "Announce: <one short sentence naming the rule>\n"
                "No extra text."
            )
        else:
            return (
                ...
                "Only test this turn, no rule announcement, no extra text. Please ensure to only use integers, not decimals."
            )
